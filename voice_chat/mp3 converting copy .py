...
# convert bit to mp3 file
# Configure the encoder:
encoder = lameenc.Encoder()
encoder.set_bit_rate(128)        # in kbps
...
# compressed = np.fromiter(compression(bit_array), dtype='U1')
...
system.receiver.rx_buffer_size = int(2.5e5)
...
## convert back to mp3
# 1) Pack bits → bytes
byte_arr = np.packbits(b, bitorder='big')

# 2) Get a bytes object
mp3_bytes = byte_arr.tobytes()

# 3) Write out the MP3
with open("restored.mp3", "wb") as f:
    f.write(mp3_bytes)


print("Wrote restored.mp3:", len(mp3_bytes), "bytes")
...
# print(f"Same signal received? {decomp == bit_array}")
...
y, sr = librosa.load('restored.mp3', sr=None)  # decode to PCM
sd.play(y, sr)
sd.wait()
